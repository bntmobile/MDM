...
namespace BigQuery
{
    public partial class Form1 : Form
    {
        ...
        public static List<string> lstTblsNoBorrar = new List<string>();
        public static List<string> lstTblsBorrar = new List<string>();
        public static List<string> lstTblsAll = new List<string>();
        ...
        public static string sBucketPathMain   = ConfigurationManager.AppSettings["BucketPathMain"];
        public static string sBucketPathCsv    = ConfigurationManager.AppSettings["BucketPathCsv"];
        public static string sLocalPathWorking = ConfigurationManager.AppSettings["LocalPathWorking"];
        public static string sWorkingPath = @sLocalPathWorking + @"\" + @sBucketPathCsv + @"\";
        ...
        public static TextWriterTraceListener twl = new TextWriterTraceListener(sWorkingPath + LogFileName);
        ...
        public static void LoadBq()
        {
            ...
            {
                if (bqcmd.Contains("DELETE"))
                {
                    DateTime ayer = DateTime.Now.AddDays(-2);
                    DateTime now = DateTime.Now.AddDays(1);
                    String FechaAyer = ayer.Year.ToString() + ayer.Month.ToString().PadLeft(2, '0') + ayer.Day.ToString().PadLeft(2, '0');
                    String FechaHoy = now.Year.ToString() + now.Month.ToString().PadLeft(2, '0') + now.Day.ToString().PadLeft(2, '0');
                    String cadena = bqcmd + FechaAyer + " AND " + FechaHoy+"'";
                    EjecutarCmd(cadena.Replace("'", "\""));
                }
                else if (bqcmd.Contains("UPDATE"))
                {
                    String sUpdate = bqcmd.Replace("'", "\"").Replace("{", "\'");
                    EjecutarCmd(sUpdate);
        ...
        public static bool InicializaVariables()
        {
            ...
            foreach (string key in ConfigurationManager.AppSettings)
             {
                    if (key.StartsWith("TablaNoBorrar"))
                    {
                        string value = ConfigurationManager.AppSettings[key];
                        lstTblsNoBorrar.Add(value);
                    }
            }
            foreach (string key in ConfigurationManager.AppSettings)
            {
                    if (key.StartsWith("TablaBorrar"))
                    {
                        string value = ConfigurationManager.AppSettings[key];
                        lstTblsBorrar.Add(value);
                    }
            }
                lstTblsAll.AddRange(lstTblsBorrar);
                lstTblsAll.AddRange(lstTblsNoBorrar);
                bandera = true;
            }
            catch (Exception ex)
            {
        ...
        public static void EjecutarCmd(string comando)
            ...
            try
            {
                ...
                cmd.StartInfo.WorkingDirectory =sWorkingPath;
        ...
        public static void BigQueryClean(string sProjectID, string sDatasetId, GoogleCredential credential)
        {
            ...
            try
            {
                ...
                {
                    if (table.Resource.Type == "TABLE")
                    {
                        lstTblsFinal.Add(table.Reference.TableId);
                    }
                }
                lstTblsFinal.RemoveAll(item => !lstTblsAll.Contains(item));

                foreach (var tblNoBorrar in lstTblsNoBorrar)
                {
                    lstTblsFinal.Remove(tblNoBorrar);
                }
        ...
        public static void LimpiaTmpLocal()
        {
            ...
            var dir = new DirectoryInfo(sWorkingPath);
        ...
        public static void LimpiaSed(string path)
        {
            try
            {
                ...
                foreach (var file in files)
                {
                    ...
                    patron = patron.Replace("'","\"") + file.Replace(sWorkingPath,"");
                    EjecutarCmd(patron);
        ...
        public static void BuscaArchivosGS(string sProjectID, GoogleCredential credential)
        {
           ...
           // Make an authenticated API request. 
            ...
            foreach (var bucket in storage.ListObjects(sBucketPathMain, "").Where( o => o.Name.Contains(sBucketPathCsv)))
            {
               /// Console.WriteLine( bucket.Name);               
                lstFiles.Add(bucket.Name);
                
            }
            lstFiles.Remove(sBucketPathCsv+"/");

            if (lstFiles.Count > 0)
            {
                DeleteObjectGStorage(sBucketPathMain, lstFiles, credential);
        ...
        public void ProcessFormLoadStuff()
        {
            ...
            if (lstChechedItems.Count > 0)
            {
                ...
                if (bComprime == true)
                {
                    LimpiaSed(sWorkingPath); // Limpia, comprime csvs en 7zip
                }
                if (bCargaGS == true)
                {
